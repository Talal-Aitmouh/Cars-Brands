...
const checkAnswer = () => {
    ...
    if (selectedChoice) {
        if (selectedChoice.textContent === correctAnswer) {
            displayAlert("Correct Answer!", false); // Pass false for correct answer
            selectedChoice.classList.add('correct-choice');
            score++;
        } else {
            displayAlert(`Wrong Answer! Try again later.`, true); // Pass true for wrong answer
...
const displayAlert = (msg, isWrong) => {
    ...
    if (isWrong) {
        alert.style.backgroundColor = "rgba(225, 95, 95, 0.489)"; // Red background for wrong answer
    } else {
        alert.style.backgroundColor = "rgba(109, 214, 109, 0.539)"; // Green background for correct answer
    }
    setTimeout(()=>{
